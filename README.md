...
Prebuilt binaries for Linux, macOS and Windows (x86_64 and arm64, except Windows which is x86_64 only) are published on [the releases page](https://github.com/otobrglez/cebelca-cli/releases/latest). The installers below pick the right one for your machine, drop the binary in `$CARGO_HOME/bin` (`~/.cargo/bin` by default) and add that to your `PATH`:

```sh
# macOS / Linux
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/otobrglez/cebelca-cli/releases/latest/download/cebelca-cli-installer.sh | sh
```

```powershell
# Windows
powershell -c "irm https://github.com/otobrglez/cebelca-cli/releases/latest/download/cebelca-cli-installer.ps1 | iex"
```

The release archives are named after the package (`cebelca-cli-<version>-<target>.tar.xz` / `.zip`); the executable inside is `ceb`. Each archive ships with a `.sha256` alongside it.

Linux builds link against glibc 2.35 or newer (they are built on Ubuntu 22.04), so on older distributions or Alpine, build from source instead — see [Development](#development).
...
Releases are built by [`dist`](https://github.com/axodotdev/cargo-dist). Two workflows live in `.github/workflows`:

- `ci.yml` — hand-written; runs `cargo fmt --check`, `cargo clippy -D warnings` and `cargo test` on every push and pull request.
- `release.yml` — **generated by `dist`**. Don't hand-edit it: change `dist-workspace.toml` (targets, installers) and re-run `dist init --yes`, which regenerates the workflow.

Cutting a release means pushing a version tag; the workflow does the rest (build all targets, make archives + checksums + installers, create the GitHub Release):

```sh
# bump `version` in Cargo.toml, update CHANGELOG.md, commit and push to master first
git tag v0.1.1
git push --tags
```

To check the release config without pushing anything:

```sh
dist plan    # list every archive and installer CI would produce, for all targets
dist build   # actually build and package for the current machine (target/distrib/)
```

`dist` is pinned via `cargo-dist-version` in `dist-workspace.toml`. It is deliberately not in `devenv.nix` — nixpkgs lags behind the pinned version — so install it separately with `cargo install cargo-dist --locked` or the installer script from its README.
